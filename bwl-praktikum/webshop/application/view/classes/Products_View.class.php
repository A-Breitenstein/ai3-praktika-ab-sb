            ...
            <div id="product_search_button" onclick="javascript:search()"> Suchen </div>
              ...
              foreach ($this->dataMap['products'] as $product){
                echo('<div id="product_entry">
                      ...
                      <div id="product_name" class="clickable" onclick="location.href=\'products?search='.$product->getBezeichnung().'\'">Bezeichnung: '.$product->getBezeichnung().'</div>
                      <div id="product_typ">Typ: '.$product->getTyp().'</div>
                      <div id="product_id">Teile-Nr.'.$product->getTeileNr().'</div>
                      <div id="product_price" class="clickable" onclick="location.href=\'products?search='.$product->getBezeichnung().'\'">Preis: '.$product->getPreis().'<br>Beschreibung:'.$product->getBeschreibung().'</div>
                      <div id="product_add_to_basket" class="clickable" onclick="location.href=\'products?'.$this->dataMap['prev_search'].'itemID='.$product->getTeileNr().'\'" >  <img width=60% height=50% src="'.Registry::$settings['config']["IMG_PATH"].'empty_basket.gif"> </div>
            </div>');
              }
           echo('</div>');
